...
public class ImRequest {
     ...
     * @param status      在线状态
     * @param nDeviceType 终端类型
     * @param nDeviceID   终端ID
    ...
    /**
     * @param nUserID      好友的用户ID
     * @param sCommentName 新的备注名称
     * @return None
     * @brief 修改好友的备注名称
     */
    ...
    /**
     * @param szUnsharpName 查找关键字
     * @param nStartNum     开始索引
     * @param nSearchNum    查找数量
     * @return None
    ...
    /**
     * @param AvatarAddress   头像二进制数据地址.
     * @param len             头像二进制数据长度.
     * @param szExtensionName 头像文件扩展名.
     * @brief 修改自定义头像.
     */
     ...
     * @brief 游客注册
     */
    public native void ImRegisterGuest(String sNickname);
    ...
    /**
     * @param nUserID    用户ID
     * @param nStatus    用户登录状态
     * @param serverTime 登录时间
     * @param sDBID
     * @param nResult    登录结果(0 登录成功)
     * @return None
     * @brief 登录IM回调函数
     */
     ...
     * @param nUserID      用户ID
     * @param nUEType      终端类型
     * @param nUserStatus  用户在线状态 (nStatus = 1 在线, 2 离开, 3 忙碌, 4 请勿打扰, 5 隐身)
     * @param szStatusDesc 状态描述
     * @return None
     * @brief 用户在线状态更新回调函数
     */
    ...
    /**
     * @param nUserID   用户ID
     * @param szUserXml 用户基本信息xml szUserXml格式：
     ...
     */
    private void OnImUserAvatarReport(int bSystemAvatar, long nUserID, String Filename) {
        for (int i = 0; i < mCallBacks.size(); i++) {
            WeakReference<ImRequestCallback> wf = this.mCallBacks.get(i);
            if (wf != null && wf.get() != null) {
                wf.get().OnChangeAvatarCallback(bSystemAvatar, nUserID, Filename);
            }
        }
    }
     ...
     * @brief 游客注册回调
     */
    private void OnImRegisterGuest(String sAccount, String sPwd, int ret) {
        for (int i = 0; i < this.mCallBacks.size(); i++) {
            WeakReference<ImRequestCallback> wf = this.mCallBacks.get(i);
            if (wf != null && wf.get() != null) {
                wf.get().OnImRegisterGuest(sAccount,sPwd, ret);
            }
        }
    }

    private void OnImErrMsg(String sErr) {
    	Log.e("ImRequest", sErr);
    }
