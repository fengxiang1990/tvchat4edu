...
public class VideoRequest {
    ...
    /**
     * @param szSessionID 会话ID
     * @param nUserID     被邀请用户ID
     * @param szDeviceID  nUserID的视频设备ID
     * @return None
     * @brief 邀请用户视频聊天
     */
    public native void VideoInviteChat(String szSessionID, long nUserID, String szDeviceID, String data);

    /**
     * @param szSessionID 会话ID
     * @param nUserID     邀请用户ID
     * @param szDeviceID  nUserID的视频设备ID
     * @return None
     * @brief 接受用户视频聊天邀请
     */
     ...
     * @brief 拒绝用户视频聊天邀请
     */
    public native void VideoRefuseChat(String szSessionID, long nUserID, String szDeviceID, String data);
    ...
    /**
     * @param nGroupType  组类型, 0为点对点, 4为会议
     * @param nGroupID    组ID
     * @param nDeviceType 视频设备类型
     * @param nUserID     目标用户ID
     * @param szDeviceID  nUserID的视频设备ID
     * @param VideoPlayer 需要构建一个VideoPlayer对象
     * @return None
     * @brief 打开视频设备
     */
     ...
     */
    private void OnVideoChatInvite(String szSessionID, long nUserID, String szDeviceID, String data) {
        for (int i = 0; i < mCallBacks.size(); i++) {
            WeakReference<VideoRequestCallback> wf = mCallBacks.get(i);
            if (wf != null && wf.get() != null) {
                wf.get().OnVideoChatInviteCallback(szSessionID, nUserID, szDeviceID, data);
            }
        }
    }
     ...
     */
    private void OnVideoChatRefused(String szSessionID, long nUserID, String szDeviceID,String data) {
        for (int i = 0; i < mCallBacks.size(); i++) {
            WeakReference<VideoRequestCallback> wf = mCallBacks.get(i);
            if (wf != null && wf.get() != null) {
                wf.get().OnVideoChatRefused(szSessionID, nUserID, szDeviceID,data);
            }
        }
    }
