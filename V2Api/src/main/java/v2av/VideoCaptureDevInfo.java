...
public class VideoCaptureDevInfo {
    ...
    private void AddDeviceInfo(VideoCaptureDevice newDevice, Camera.Parameters parameters) {
        ...
        for (Integer fmt : newDevice.previewformats) {
            Log.i(TAG, "supported format " + fmt);
        }
        List<Size> sizes = parameters.getSupportedPreviewSizes();
        if (sizes != null) {
            for (Size s : sizes) {
                Log.d(TAG, "sizes CaptureCapability:" + s.width + " " + s.height);
                newDevice.capabilites.add(new CaptureCapability(s.width, s.height));
            }
        }
        Collections.sort(newDevice.capabilites);
        for (CaptureCapability cap : newDevice.capabilites) {
            Log.d(TAG, "capabilites CaptureCapability:" + cap.width + " " + cap.height);
        }
        ...
        for (int[] range : fps_ranges) {
            ...
            Log.d(TAG, "getSupportedPreviewFpsRange : " + strRange);
        }

        @SuppressWarnings("deprecation")
        List<Integer> frameRates = parameters.getSupportedPreviewFrameRates();
        if (frameRates == null) {
            newDevice.framerates.add(10);
        } else {
