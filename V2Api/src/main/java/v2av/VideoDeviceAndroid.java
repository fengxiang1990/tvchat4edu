...
public class VideoDeviceAndroid {
	...
	public VideoDeviceAndroid() {
		VideoNative tmp = new VideoNative();
		tmp.enablehardwarecodec(VideoEncoder.GetMediaCodecEnabled(),
				VideoPlayer.GetMediaCodecEnabled());
	}
	...
	private String GetVideoDevInfo() {
		...
		if (mCapDevInfo.deviceList == null) {
			mCapDevInfo.deviceList = new ArrayList<>();
		}
		...
		for (int i = 0; i < devNum; i++) {
			...
			sb.append("' fps='");
			// if(frameSize > 0 && dev.framerates.get(frameSize - i - 1) !=
			// null){
			// sb.append(dev.framerates.get(frameSize - i - 1));
			// }
			if (frameSize > 0 && dev.framerates.get(frameSize - 1) != null) {
				sb.append(dev.framerates.get(frameSize - 1));
			} else {
				sb.append(15);
			}
