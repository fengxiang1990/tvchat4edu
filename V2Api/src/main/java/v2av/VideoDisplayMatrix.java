...
public class VideoDisplayMatrix {
	...
	public void setViewSize(int width, int height , float scale) {
		mViewWidth = width;
		mViewHeight = height;
		zoomOut(scale);
		center(true, true);
	...
	protected void zoomTo(float pscale, float pcenterX, float pcenterY, float pdurationMs) {
		centerX = pcenterX;
		centerY = pcenterY;
	...
	private void getProperBaseMatrix(Matrix matrix) {
		...
		// a small icon.
		float widthScale = (float) ((float) mViewWidth / w);// Math.min((float)((float)mViewWidth
															// / w), 3.0f);
		float heightScale = (float) ((float) mViewHeight / h);// Math.min((float)((float)mViewHeight
																// / h), 3.0f);
		...
		matrix.postTranslate((mViewWidth - w * scale) / 2F, (mViewHeight - h * scale) / 2F);
	}
	...
	protected void center(boolean horizontal, boolean vertical) {
		...
		RectF rect = new RectF(0, 0, mRoBitmap.getBitmap().getWidth(), mRoBitmap.getBitmap().getHeight());
		m.mapRect(rect);
