...
public class ClipAvatarLayout extends RelativeLayout implements OnTouchListener, OnClickListener {
	private static final float MAX_SCALE = 10.0f;
	private static final int DEFAULT_SIZE = BitmapUtil.getAvatarSize();
	...
	public ClipAvatarLayout(Context context) {
		super(context);
		init(context);
	}

	public ClipAvatarLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public ClipAvatarLayout(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(Context context) {
		...
		if(dm.widthPixels > dm.heightPixels){
			mClipViewWidth = dm.heightPixels * 2 / 3;
			mClipViewHeight = dm.heightPixels * 2 / 3;
		} else {
	...
	public void rotate(float degree) {
		...
		matrix.postRotate(degree, (rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2);
	...
	public void setSourceImage(Bitmap bitmap, Window window) {
		...
		mBitmap = Bitmap.createBitmap(bitmap, 0, 0, srcWidth, srcHeight, matrix, true);
	...
	public Bitmap getBitmap() {
		...
		matrix.postScale(scale, scale);
		Bitmap bitmap = Bitmap.createBitmap(mBitmap, (int) (rect.left), (int) (rect.top),
				(int) (rect.right - rect.left), (int) (rect.bottom - rect.top), matrix, false);
		return bitmap;
	}
	...
	private Rect getClipRect() {
		...
		Point top = getCloest(new Point(scale.left, scale.top), new Point((int) insert.left, (int) insert.top),
				new Point((int) insert.left, (int) insert.bottom), new Point((int) insert.right, (int) insert.top),
				new Point((int) insert.right, (int) insert.bottom));
		Point bottom = getFarthest(new Point(scale.left, scale.top), new Point((int) insert.left, (int) insert.top),
				new Point((int) insert.left, (int) insert.bottom), new Point((int) insert.right, (int) insert.top),
				new Point((int) insert.right, (int) insert.bottom));
	...
	private void scaleFit(Bitmap bitmap, float width, float height) {
		...
		if (sourceHeight < height && sourceWidth < width) {
			if (disHeight < disWidth) {
				matrix.setScale(height / sourceHeight, height / sourceHeight, getWidth() / 2, 0);

			} else {
				matrix.setScale(width / sourceWidth, width / sourceWidth, getWidth() / 2, 0);
			}
		}
