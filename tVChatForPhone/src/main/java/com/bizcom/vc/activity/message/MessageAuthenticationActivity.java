...
public class MessageAuthenticationActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		isFriendAuthentication = getIntent().getBooleanExtra("isFriendActivity", true);
		if (isFriendAuthentication) {
			VerificationProvider.updateCrowdQualicationReadState(VerificationMessageType.CONTACT_TYPE, ReadState.READ,
					null, null);
			changeMessageAuthenticationListView();
		} else {
			VerificationProvider.updateCrowdQualicationReadState(VerificationMessageType.CROWD_TYPE, ReadState.READ,
					null, null);
	...
	private void initPrompt() {
		boolean showFriendNotification = VerificationProvider.getUNReandMessage(true);
		V2Log.d(TAG, "好友验证消息是否有未读的 : " + showFriendNotification);
		...
		boolean showCrwodNotification = VerificationProvider.getUNReandMessage(false);
		V2Log.d(TAG, "群验证消息是否有未读的 : " + showCrwodNotification);
	...
	private void connectView() {
		tvMessageBack = (TextView) findViewById(R.id.ws_common_activity_title_left_button);
        tvMessageBack.setBackgroundResource(R.drawable.title_bar_back_button_selector);

		tvCompleteRight = (TextView) findViewById(R.id.ws_common_activity_title_right_button);
		tvCompleteRight.setText(getResources().getString(R.string.friendManagementActivity_titlebar_right_text));
		...
		titleContent.setText(getResources().getString(R.string.group_create_group_qualification));
	...
	public void requestUpdateConversation() {
		...
		i.putExtra("obj", new ConversationNotificationObject(0, Conversation.SPECIFIC_VERIFICATION_ID, false));
	...
	private void bindViewEnvent() {
		...
		rbFriendAuthentication.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
				if (arg1) {

					currentRadioType = PROMPT_TYPE_FRIEND;
					rbFriendAuthentication.setTextColor(Color.rgb(255, 255, 255));
					rbGroupAuthentication.setTextColor(getResources().getColor(R.color.button_text_color_blue));
					isFriendAuthentication = true;
					changeMessageAuthenticationListView();
					VerificationProvider.updateCrowdQualicationReadState(VerificationMessageType.CONTACT_TYPE,
		...
		rbGroupAuthentication.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
				if (arg1) {
					currentRadioType = PROMPT_TYPE_GROUP;
					rbGroupAuthentication.setTextColor(Color.rgb(255, 255, 255));
					rbFriendAuthentication.setTextColor(getResources().getColor(R.color.button_text_color_blue));
					isFriendAuthentication = false;
					changeMessageAuthenticationListView();
					updateTabPrompt(PROMPT_TYPE_GROUP, false);
					VerificationProvider.updateCrowdQualicationReadState(VerificationMessageType.CROWD_TYPE,
							ReadState.READ, null, null);
				}
			}
		});

		lvMessageAuthentication.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if (isFriendAuthentication) {
					if (isFriendInDeleteMode) {
						Toast.makeText(mContext, R.string.crowd_message_deletion_mode_quit_required, Toast.LENGTH_SHORT)
								.show();
						return;
					}
					...
					intent.putExtra("uid", data.remoteUserID);
					intent.putExtra("remoteNickName", data.name);
					intent.putExtra("authenticationMessage", data.authenticationMessage);
					intent.putExtra("state", data.state);
					intent.putExtra("fromActivity", "MessageAuthenticationActivity");
					// 别人加我：允许任何人：0已添加您为好友，需要验证：1未处理，2已同意，3已拒绝
					// 我加别人：允许认识人：4你们已成为了好友，需要验证：5等待对方验证，4被同意（你们已成为了好友），6拒绝了你为好友
					switch (data.state) {
					case 5:
						intent.setClass(MessageAuthenticationActivity.this, ContactDetail2.class);
						break;
					case 0:
					case 1:
					case 2:
					...
					VMessageQualification msg = (VMessageQualification) mMessageList.get(position).obj;
					if (msg.getType() == VMessageQualification.Type.CROWD_INVITATION) {
						...
						Intent i = new Intent();
						i.setClass(MessageAuthenticationActivity.this, CrowdApplicantDetailActivity.class);
						i.putExtra("cid", amsg.getCrowdGroup().getGroupID());
						i.putExtra("aid", amsg.getApplicant().getmUserId());
		...
		lvMessageAuthentication.setOnItemLongClickListener(new OnItemLongClickListener() {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case FRIEND_AUTHENTICATION_RESULT:
			if (data != null) {
                ...
                List<FriendMAData> friendMADatas = VerificationProvider.loadFriendsVerifyMessages();
                if(friendMADatas != null){
				    friendMADataList.addAll(friendMADatas);
				    firendAdapter.notifyDataSetChanged();
                }
			}
			break;
	...
	private void checkVerificationMessage() {
		...
		try {
			...
			String sql = "select * from " + AddFriendHistroysHandler.tableName + " order by SaveDate desc limit 1";
			cursor = AddFriendHistroysHandler.select(mContext, sql, new String[] {});
			if ((cursor == null || !cursor.moveToNext()) && nestQualification == null) {
	...
	private void loadFriendMessage() {
		new AsyncTask<Void, Void, List<FriendMAData>>() {
			...
			protected List<FriendMAData> doInBackground(Void... arg0) {
				List<FriendMAData> tempMessageAuthenticationDataList = VerificationProvider.loadFriendsVerifyMessages();
	...
	private void startCrowdInvitationDetail(VMessageQualification msg) {
		...
		if (imsg.getQualState() == QualificationState.BE_ACCEPTED) {
			if (GlobalHolder.getInstance().getGroupById(imsg.getCrowdGroup().getGroupID()) != null) {
				i.putExtra("obj", new ConversationNotificationObject(Conversation.TYPE_GROUP,
						imsg.getCrowdGroup().getGroupID(), false));
			...
			CrowdGroup group = (CrowdGroup) GlobalHolder.getInstance().getGroupById(imsg.getCrowdGroup().getGroupID());
			Crowd crowd;
			if (group != null) {
				crowd = new Crowd(group.getGroupID(), group.getOwnerUser(), group.getName(), group.getBrief());
				crowd.setAnnounce(group.getAnnouncement());
				crowd.setAuth(group.getAuthType().intValue());
			} else {
				CrowdGroup crowdGroup = imsg.getCrowdGroup();
				crowd = new Crowd(crowdGroup.getGroupID(), crowdGroup.getOwnerUser(), crowdGroup.getName(),
						crowdGroup.getBrief());
	...
	class FriendMessageAdapter extends BaseAdapter {
		class ViewTag {
			// R.id.head_image
			CustomAvatarImageView ivHeadImage;
			// R.id.name
			TextView tvName;
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			...
			if (arg1 == null) {
				...
				viewTag = new ViewTag();
				viewTag.ivHeadImage = (CustomAvatarImageView) arg1.findViewById(R.id.ws_common_avatar);
				...
				viewTag.bAccess = (TextView) arg1.findViewById(R.id.qualification_msgconfirm_button);
				viewTag.tvAccessOrNo = (TextView) arg1.findViewById(R.id.qualification_msg_res);
				viewTag.ibDelete = (ImageView) arg1.findViewById(R.id.qualification_msg_delete_left_button);
				viewTag.bSuerDelete = (TextView) arg1.findViewById(R.id.qualification_msg_delete_button);
				arg1.setTag(viewTag);
			} else {
			...
			case 0:
				...
				break;
			case 1:
				viewTag.tvAuthenticationMessage.setText(data.authenticationMessage);
			...
			viewTag.bAccess.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					Intent intent = new Intent(context, InputRemarkActivity.class);
			...
			viewTag.bSuerDelete.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					// 库中删除
					String sql = "delete from " + tableName + " where _id=" + list.get(position).dbRecordIndex;
	...
	class ListItemWrapper {
		...
		public ListItemWrapper(Object obj, boolean showLeftDeleteButton, boolean showDeleteButton) {
			super();
			this.obj = obj;
	...
	class GroupMessageAdapter extends BaseAdapter {
		...
		class ViewItem {
			...
			CustomAvatarImageView mMsgBanneriv;
			TextView mNameTV;
			TextView mContentTV;
			TextView mRes;
			View mAcceptButton;
		...
		public long getItemId(int position) {
			return ((VMessageQualification) mMessageList.get(position).obj).getId();
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			ViewItem item = null;
			if (convertView == null) {
				convertView = layoutInflater.inflate(R.layout.qualification_message_adapter_item, null);
				item = new ViewItem();
				item.mDeleteHintButton = convertView.findViewById(R.id.qualification_msg_delete_left_button);
				// 拒绝理由显示，暂不用
				...
				item.mMsgBanneriv = (CustomAvatarImageView) convertView.findViewById(R.id.ws_common_avatar);
				item.mNameTV = (TextView) convertView.findViewById(R.id.ws_common_conversation_layout_topContent);
				item.mContentTV = (TextView) convertView.findViewById(R.id.ws_common_conversation_layout_belowContent);
				item.mRes = (TextView) convertView.findViewById(R.id.qualification_msg_res);
				item.mAcceptButton = convertView.findViewById(R.id.qualification_msgconfirm_button);
				item.mAcceptButton.setOnClickListener(mAcceptButtonListener);
				item.mDeleteButton = convertView.findViewById(R.id.qualification_msg_delete_button);
		...
		private void updateViewItem(ListItemWrapper wrapper, ViewItem item) {
			...
			if (msg.getType() == VMessageQualification.Type.CROWD_INVITATION) {
				...
				if (vqic.getInvitationUser() != null) {
					...
					item.mContentTV.setText(name + mContext.getText(R.string.crowd_invitation_content));
				}
				...
				item.mMsgBanneriv.setImageBitmap(vqac.getApplicant().getAvatarBitmap());
			...
			if (wrapper.showDeleteButton && isGroupInDeleteMode) {
				...
				if (msg.getType() == VMessageQualification.Type.CROWD_APPLICATION)
					updateApplyMessageView(item, (VMessageQualificationApplicationCrowd) msg);
				else
					updateInviteMessageView(item, (VMessageQualificationInvitationCrowd) msg);
			}
		}

		private void updateApplyMessageView(ViewItem item, VMessageQualificationApplicationCrowd vqac) {
			if (vqac.getQualState() == QualificationState.ACCEPTED) {
				...
				item.mContentTV.setText(String.format(res.getString(R.string.crowd_invitation_apply_join),
				...
				item.mContentTV.setText(String.format(res.getString(R.string.crowd_invitation_reject_join),
						vqac.getCrowdGroup().getName()));
			} else if (vqac.getQualState() == QualificationState.BE_ACCEPTED) {
				item.mRes.setVisibility(View.INVISIBLE);
				item.mAcceptButton.setVisibility(View.INVISIBLE);
				// item.mRejectContentTV.setVisibility(View.GONE);
				item.mContentTV.setText(String.format(res.getString(R.string.crowd_invitation_accept_join),
						vqac.getCrowdGroup().getName()));
		...
		private void updateInviteMessageView(ViewItem item, VMessageQualificationInvitationCrowd vqic) {
		...
		private OnClickListener mAcceptButtonListener = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (msg.getType() == VMessageQualification.Type.CROWD_INVITATION) {
					CrowdGroup cg = ((VMessageQualificationInvitationCrowd) msg).getCrowdGroup();
					if (message.getQualState().intValue() != msg.getQualState().intValue()) {
						...
						intent.putExtra("group",
								new GroupUserObject(V2GlobalConstants.GROUP_TYPE_CROWD, cg.getGroupID(), -1));
						sendBroadcast(intent);
						...
						Crowd crowd = new Crowd(cg.getGroupID(), cg.getOwnerUser(), cg.getName(), cg.getBrief());
						crowdService.acceptInvitation(crowd,
								new HandlerWrap(mLocalHandler, ACCEPT_INVITATION_DONE, cg));
						WaitDialogBuilder.showNormalWithHintProgress(mContext);
					}
				} else if (msg.getType() == VMessageQualification.Type.CROWD_APPLICATION) {
					VMessageQualificationApplicationCrowd vqac = ((VMessageQualificationApplicationCrowd) msg);
					Group group = GlobalHolder.getInstance().getGroupById(vqac.getCrowdGroup().getGroupID());
					if (group != null) {
						if (message.getQualState().intValue() != msg.getQualState().intValue()) {
							...
							crowdService.acceptApplication(vqac.getCrowdGroup(), vqac.getApplicant(),
									new HandlerWrap(mLocalHandler, ACCEPT_APPLY_DONE, group));
							WaitDialogBuilder.showNormalWithHintProgress(mContext);
						}
					} else {
						handleFailedDone();
	...
	private void handleAcceptDone(Type type, long groupID, long userID) {
		...
		VMessageQualification vq = VerificationProvider.queryCrowdQualMessageByCrowdId(userID, groupID);
		for (ListItemWrapper wrapper : mMessageList) {
			VMessageQualification message = (VMessageQualification) wrapper.obj;
		...
		public void onReceive(Context arg0, Intent intent) {
			if (JNIService.JNI_BROADCAST_NEW_QUALIFICATION_MESSAGE.equals(intent.getAction())) {
				if (currentRadioType == PROMPT_TYPE_GROUP) {
					...
					VMessageQualification msg = VerificationProvider.queryCrowdQualMessageById(msgId);
			...
			} else if (JNIService.JNI_BROADCAST_GROUP_JOIN_FAILED.equals(intent.getAction())) {
				...
				for (int i = 0; i < mMessageList.size(); i++) {
					...
					if (message.getType() == Type.CROWD_APPLICATION) {
						...
						if (apply.getCrowdGroup() != null) {
							if (apply.getCrowdGroup().getGroupID() == cid) {
								location = i;
								break;
							}
						} else
						...
						if (invite.getCrowdGroup() != null) {
							if (invite.getCrowdGroup().getGroupID() == cid) {
								location = i;
								break;
							}
						} else
                ...
                if(g.getGroupType() == V2GlobalConstants.GROUP_TYPE_CROWD){
                    ...
                    for (int i = 0; i < mMessageList.size(); i++) {
                        ...
                        VMessageQualification message = (VMessageQualification) wrapper.obj;
                        if (message.getmCrowdGroup().getGroupID() == g.getGroupID()) {
                            message.setmCrowdGroup(crowd);
                            if (groupAdapter != null) {
                                groupAdapter.notifyDataSetChanged();
                            }
                            break;
				...
				for (int i = 0; i < mMessageList.size(); i++) {
					...
					if (obj.getmType() == V2GlobalConstants.GROUP_TYPE_DEPARTMENT) {
						if (message.getType() == Type.CROWD_APPLICATION
								&& ((VMessageQualificationApplicationCrowd) message).getApplicant().getmUserId() == obj
										.getmUserId()
								|| message.getType() == Type.CROWD_INVITATION
										&& ((VMessageQualificationInvitationCrowd) message).getInvitationUser()
												.getmUserId() == obj.getmUserId()) {
							mMessageList.remove(i);
						}
					} else {
						if (message.getmCrowdGroup().getGroupID() == obj.getmGroupId()
								&& (message.getType() == Type.CROWD_APPLICATION
										&& ((VMessageQualificationApplicationCrowd) message).getApplicant()
												.getmUserId() == obj.getmUserId()
										|| message.getType() == Type.CROWD_INVITATION
												&& ((VMessageQualificationInvitationCrowd) message).getInvitationUser()
														.getmUserId() == obj.getmUserId())) {
	...
	class FriendAuthenticationBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context arg0, Intent arg1) {
			if (arg1.getAction().equals(JNIService.JNI_BROADCAST_CONTACTS_AUTHENTICATION)) {
				loadFriendMessage();
				if (currentRadioType != PROMPT_TYPE_FRIEND)
					updateTabPrompt(PROMPT_TYPE_FRIEND, true);
			} else if (arg1.getAction().equals(JNIService.JNI_BROADCAST_GROUP_USER_REMOVED)) {
				GroupUserObject guo = (GroupUserObject) arg1.getExtras().get("obj");
				if (guo == null) {
					V2Log.e(TAG,
		...
		public void handleMessage(Message msg) {
			...
			case ACCEPT_INVITATION_DONE:
				...
				if (jni.getResult().ordinal() == JNIResponse.Result.SUCCESS.ordinal()) {
					...
					handleAcceptDone(Type.CROWD_INVITATION, crowd.getGroupID(), crowd.getOwnerUser().getmUserId());
				} else {
					if (jni.getResult().ordinal() != JNIResponse.Result.SERVER_REJECT.ordinal()) {
						Toast.makeText(mContext, R.string.common_networkConnection_timeout, Toast.LENGTH_SHORT).show();
					}
				}
				break;
			case ACCEPT_APPLY_DONE:
				...
				if (applyJni.getResult().ordinal() == JNIResponse.Result.SUCCESS.ordinal()) {
					...
					handleAcceptDone(Type.CROWD_APPLICATION, obj.getGroupID(), obj.getUserID());
				} else {
					Toast.makeText(mContext, R.string.common_networkConnection_timeout, Toast.LENGTH_SHORT).show();
				}
				break;
