...
public class BitmapUtils {
	...
	private static Bitmap compressImage(float size, int orientation, boolean scale, boolean isLow,
		...
		try {
			...
			options.inSampleSize = calculateInSampleSize((float) options.outWidth, (float) options.outHeight, destWidth,
					destHeight);
			...
			if (bmp == null)
				return bmp;
			scaledBitmap = Bitmap.createBitmap((int) destWidth, (int) destHeight,
			...
			if (t != null)
				Log.e("exception",
						"BitmapUtil.decodeBitmapFromPath(String path, Activity activity) Exception " + t.getMessage());
			return null;
		} finally {
