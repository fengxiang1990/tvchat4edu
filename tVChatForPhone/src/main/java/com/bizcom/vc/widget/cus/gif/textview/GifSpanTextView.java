...
public class GifSpanTextView extends TextView {
	...
	public void setGifText(CharSequence text) {
	...
	public void setText(CharSequence text, BufferType type) {
		...
		if (!TextUtils.isEmpty(oldText) && oldText instanceof Spannable) {
			...
			final GifImageSpan[] spans = sp.getSpans(0, sp.length(),
					GifImageSpan.class);
			...
			final GifSpanChangeWatcher[] watchers = sp.getSpans(0, sp.length(),
					GifSpanChangeWatcher.class);
		...
		if (!TextUtils.isEmpty(text) && text instanceof Spannable) {
			...
			sp.setSpan(mGifSpanChangeWatcher, 0, text.length(),
	...
	private GifImageSpan getImageSpan(Drawable drawable) {
		...
		if (!TextUtils.isEmpty(text)) {
			if (text instanceof Spanned) {
				...
				GifImageSpan[] spans = spanned.getSpans(0, text.length(),
						GifImageSpan.class);
