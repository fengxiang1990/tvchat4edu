...
package com.bizcom.vc.widget.cus.roundedimageview;
...
public class RoundedImageView extends ImageView {
	...
	public User.Status mUserStatus;
	...
	public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mCornerRadii[Corner.TOP_LEFT] = a.getDimensionPixelSize(R.styleable.RoundedImageView_riv_corner_radius_top_left,
				-1);
	...
	private void drawPress(Canvas canvas) {
		if (mPressPaint == null) {
			...
			mPressPaint.setColor(getResources().getColor(R.color.common_item_text_color_gray));
			mPressPaint.setAlpha(0);
			mPressPaint.setFlags(Paint.ANTI_ALIAS_FLAG);

			mPressAlpha = 155;
			mRadius = DensityUtils.dip2px(getContext(), 10);
			rectF = new RectF(0, 0, mWidth, mHeight);
		}

		if (mUserStatus == User.Status.OFFLINE || mUserStatus == User.Status.HIDDEN) {
			mPressPaint.setAlpha(mPressAlpha);
		} else {
			mPressPaint.setAlpha(0);
		}
		canvas.drawCircle(mWidth / 2, mWidth / 2, mWidth / 2, mPressPaint);
