...
public class TouchImageView extends V2ImageView {
    ...
    public void setImageBitmap(Bitmap bm, BitmapResultCallBack callBack) {
        super.setImageBitmap(bm, callBack);
        savePreviousImageValues();
        fitImageToView();
    }
...
//          mScaleDetector.onTouchEvent(event);
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_POINTER_DOWN:
                    ...
                    midPoint(mid, event);
                    setState(State.ZOOM);
                    // 在有些设备上左右滑动时会和父控件viewpager滑动冲突，导致
                    // 回到ACTION_CANNEL,所以如果当处在ZOOM状态时调用该函数向
                    // 父控件说明现在不要干扰我的行为
                ...
                case MotionEvent.ACTION_MOVE:
                    if (state == State.ZOOM) {
                        ...
                        scaleImage(scale, mid.x, mid.y, true);
                        oldDist = newDist;
                    }
                    break;
